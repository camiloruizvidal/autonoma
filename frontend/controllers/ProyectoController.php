   ...
   public function actionPublic($id)
      {
          ...
          $model = new Proyecto();
           $params = [':id' => $_GET['id']];
            $actualizar = Yii::$app->db->createCommand('UPDATE  proyecto SET estado = 1 WHERE idproyecto=:id')
              ->bindValues($params);
              $actualizar->execute();
              \Yii::$app->session->setFlash('success', 'Proyecto publicado');
               return $this->redirect(['index', 'id' => $model->idproyecto]);
    ...
    public function actionCreate()
    {
           ...
           if (!empty($model->file1)) {
             $model->file1->saveAs('articulo/'.$archivo.'.'.$model->file->extension );
             $model->articulo = 'articulo/'.$archivo.'.'.$model->file->extension ;
           }
